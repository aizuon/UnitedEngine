...
namespace fva
{
	...
	template <class T>
	template <typename... Args>
	Handle<T> SwapArray<T>::add(Args&&... args)
	{
	...
	template <class T>
	template <typename... Args>
	void SwapArray<T>::add(Handle<T>& slot, Args&& ...args)
	{
		// Add object and reverse index
		m_data.emplace_back(args...);
	...
	template <class T>
	Handle<T> SwapArray<T>::getFreeSlot()
	{
		return Handle<T>(getFreeSlotIndex(), *this);
	}

	template <class T>
	void SwapArray<T>::remove(Handle<T>& handle)
	{
	...
	template <class T>
	T& SwapArray<T>::operator[](const Handle<T>& handle)
	{
	...
	template <class T>
	T& SwapArray<T>::operator[](uint32_t index)
	{
	...
	template <class T>
	typename std::vector<T>::iterator SwapArray<T>::begin()
	{
		return m_data.begin();
	}

	template <class T>
	typename std::vector<T>::iterator SwapArray<T>::end()
	{
		return m_data.end();
	}

	template <class T>
	typename std::vector<T>::const_iterator SwapArray<T>::begin() const
	{
		return m_data.begin();
	}

	template <class T>
	typename std::vector<T>::const_iterator SwapArray<T>::end() const
	{
		return m_data.end();
	}

	template <class T>
	uint32_t SwapArray<T>::size() const
	{
		return static_cast<uint32_t>(m_data.size());
	}

	template <class T>
	void SwapArray<T>::clear()
	{
		m_data.clear();
		m_index.clear();
	...
	template <class T>
	std::vector<T>& SwapArray<T>::getData()
	{
		return m_data;
	}

	template <class T>
	const std::vector<T>& SwapArray<T>::getConstData() const
	{
		return m_data;
	}

	template <class T>
	uint32_t SwapArray<T>::getFreeSlotIndex()
	{
		// Compute data_index (index in _data) and init index (index for access from outside)
		uint32_t data_index = static_cast<uint32_t>(m_data.size());
