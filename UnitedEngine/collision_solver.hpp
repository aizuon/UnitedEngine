...
namespace up
{
	...
	public:
		...
		CollisionSolver(const Vec2& dimension, float body_radius, std::vector<Body>& bodies,
		                const Vec2& gravity = Vec2(0.0f, 0.0f))
			: m_gravity(gravity)
			  , m_grid(dimension, 4 * static_cast<uint32_t>(body_radius), bodies)
			  , m_swarm(16)
		...
		void update(fva::SwapArray<Body>& bodies, fva::SwapArray<SolidSegment>& segments, float dt)
		{
			...
			{
				clock_local.restart();
				swrm::WorkGroup group = m_swarm.execute([this](uint32_t id, uint32_t worker_count)
				{
					solveCollisionsSwarm(id, worker_count, m_grid.getCells());
				});
		...
		Body* getBodyAt(const Vec2& position, fva::SwapArray<Body>& bodies)
		{
			for (Body& b : bodies)
			{
	...
	private:
		...
		void solveCellCollisions(uint32_t size, std::array<Body*, GRID_CELL_SIZE>& bodies)
		{
		...
		void solveBodySegment(std::vector<SolidSegment*>& segments, std::array<Body*, GRID_CELL_SIZE>& bodies,
		                      uint32_t size)
		{
		...
		void naive_segments_bodies(std::vector<SolidSegment>& segments, std::vector<Body>& bodies)
		{
			for (SolidSegment& s1 : segments)
			{
				for (Body& b : bodies)
